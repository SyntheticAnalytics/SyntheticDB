...
from typing import List, Type, Callable, Union, TypeVar, Dict, Tuple, Iterable
...
class Table:
    ...
    def sample(self) -> Iterable[pd.DataFrame]:
        pass
        # pk = count(1)
        # for fk in self.fk_columns:
        #
        # vals = {col.name: col.sample() for col in self.dist_columns if isinstance(col, DistributionColumn)}
